...
use crate::{instructions::{calculate_health_factor, get_lamports_from_usd}, burn_tokens, withdraw_sol,Collateral, Config, SEED_CONFIG_ACCOUNT};
...
pub fn process_liquidate(
    ...
    require!(health_factor < ctx.accounts.config_account.min_health_factor, CustomError::HealthFactorAboveThreshold);
    ...
    calculate_health_factor(collateral_account, &ctx.accounts.config_account, &ctx.accounts.price_update)?;
    Ok(())
}
